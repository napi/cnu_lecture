...
public class AccessParentTest {
	...
	/**
	 * Another 클래스는 field로 AccessParent 클래스를 갖고 있다. Another 과 AccessParent 는 같은
	 * 패키지에 존재함으로, default 접근자 메서드를 사용할 수 있다.
	 */
	@Test
	public void another() {
		assertThat("Test코드를 수정해서 아래 코드를 해결할 것", another.getAccessParentNameByDefault(),
				is("Another : AccessParent_Default"));
	}

	@Test
	public void testAccessNameByProtected() {
		assertThat("Test코드를 수정해서 아래 코드를 해결할 것", accessChildren.getNameByProtected(),
				is("Child : AccessParent_Protected"));
	}
	...
	public void 상속과_형변환() throws Exception {
		...
		assertThat("parent 로 캐스팅 됐지만, 객체는 여전히 children의 속성을 갖고 있다.", accessParentCasted.getNameByProtected(),
				is("Child : AccessParent_Protected")); // 나 부모로 캐스팅 된거 아닌가??
	}
